...
document.addEventListener("DOMContentLoaded", () => {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    const enderecos = Array.from(document.querySelectorAll(".address-clone"))
      .map((frame) => {
        const tipoResidencia = frame.querySelector(
          'select[name="tipoResidencia"]'
        )?.value;
        const tipoLogradouro = frame.querySelector(
          'select[name="tipoLogradouro"]'
        )?.value;
        const logradouro = frame
          .querySelector('input[name="logradouro"]')
          ?.value.trim();
        const numero = frame
          .querySelector('input[name="numero"]')
          ?.value.trim();
        ...
        const observacoes = frame
          .querySelector('textarea[name="observacoes"]')
          ?.value.trim();
        const cobranca =
          frame.querySelector('input[name="endereco-cobranca"]')?.checked ||
          false;
        const entrega =
          frame.querySelector('input[name="endereco-entrega"]')?.checked ||
          false;
    ...
    const cartoesCredito = Array.from(document.querySelectorAll(".card-clone"))
      .map((frame) => {
        ...
        const numeroCartao = frame
          .querySelector('input[name="numero-cartao"]')
          ?.value.trim();
        const nomeImpresso = frame
          .querySelector('input[name="nome-titular"]')
          ?.value.trim();
        const codigoSeguranca = frame
          .querySelector('input[name="cvv"]')
          ?.value.trim();
        const preferencial =
          frame.querySelector('input[name="cartao-preferencial"]')?.checked ||
          false;
    ...
    const cliente = {
      ...
      confirmarSenha:
        document.getElementById("confirmacao-senha")?.value.trim() || "",
      telefones,
      enderecos,
      cartoesCredito,
    };
    ...
    try {
      ...
      if (response.ok) {
        const clienteSalvo = await response.json(); // pega o JSON do backend
